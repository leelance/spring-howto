...
flowableAdminApp.controller('CmmnTaskController', ['$scope', '$rootScope', '$http', '$timeout', '$location', '$routeParams', '$modal', '$translate', '$q', 'gridConstants',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate, $q, gridConstants) {
        ...
        $scope.loadTask = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-tasks/' + $routeParams.taskId
            ...
            .then(function (headers) {
                $scope.subTaskGridDefinitions = {
                    ...
                    columnDefs: [
                        ...
                        {
                            field: 'assignee',
                            displayName: headers[2],
                            width: 100,
                            cellTemplate: gridConstants.defaultTemplate
                        },
                        {
                            field: 'owner',
        ...
        $scope.loadSubTasks = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-tasks/' + $scope.task.id + '/subtasks'
            }).success(function (data, status, headers, config) {
                $scope.subTasks = data;
        ...
        $scope.loadVariables = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-tasks/' + $scope.task.id + '/variables'
            }).success(function (data, status, headers, config) {
        ...
        $scope.loadIdentityLinks = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-tasks/' + $scope.task.id + '/identitylinks'
            }).success(function (data, status, headers, config) {
        ...
        $scope.showTaskForm = function () {
            if ($scope.task.endTime) {
                $http({
                    method: 'GET',
                    url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/task-form-instance/' + $scope.task.id
                }).success(function (data, status, headers, config) {
        ...
        $scope.delegateTask = function () {
            ...
            modalInstance.result.then(function (user) {
                if (user) {
                    $scope.addAlert($translate.instant('ALERT.TASK.DELEGATED', {
                        id: $scope.task.id,
                        user: user
                    }), 'info');
                    $scope.loadTask();
            ...
            modalInstance.result.then(function (user) {
                if (user !== undefined) {
                    if (user == '') {
                        ...
                        $scope.addAlert($translate.instant('ALERT.TASK.ASSIGNED', {
                            id: $scope.task.id,
                            user: user
                        }), 'info');
                    }
...
flowableAdminApp.controller('CmmnCompleteModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
        ...
        $scope.ok = function () {
            ...
            $http({
                method: 'POST',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-tasks/' + $scope.task.id,
    ...
    ['$rootScope', '$scope', '$modalInstance', '$http', 'task', function ($rootScope, $scope, $modalInstance, $http, task) {
        ...
        $scope.ok = function () {
            ...
            $http({
                method: 'POST',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-tasks/' + $scope.task.id,
                data: {action: 'delegate', assignee: $scope.status.user}
            }).success(function (data, status, headers, config) {

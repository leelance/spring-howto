...
flowableAdminApp.controller('CmmnJobController', ['$scope', '$rootScope', '$http', '$timeout', '$location', '$routeParams', '$modal', '$translate',
    function ($scope, $rootScope, $http, $timeout, $location, $routeParams, $modal, $translate) {
        ...
        $scope.executeJob = function () {
            $http({
                method: 'POST',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-jobs/' + $scope.job.id
        ...
        $scope.moveJob = function () {
            $http({
                method: 'POST',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/move-cmmn-jobs/' + $scope.job.id + '?jobType=' + $scope.jobType.param
            }).success(function (data, status, headers, config) {
        ...
        $scope.loadJob = function () {
            ...
            $http({
                method: 'GET',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-jobs/' + $routeParams.jobId + '?jobType=' + $scope.jobType.param
            }).success(function (data, status, headers, config) {
                $scope.job = data;
                ...
                if ($scope.job.exceptionMessage) {
                    ...
                    $http({
                        method: 'GET',
                        url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-jobs/' + $scope.job.id + '/stacktrace?jobType=' + $scope.jobType.param
                    }).success(function (data, status, headers, config) {
...
flowableAdminApp.controller('DeleteCmmnJobModalInstanceCtrl',
    ['$rootScope', '$scope', '$modalInstance', '$http', 'job', 'jobType', function ($rootScope, $scope, $modalInstance, $http, job, jobType) {
        ...
        $scope.ok = function () {
            ...
            $http({
                method: 'DELETE',
                url: FlowableAdmin.Config.adminContextRoot + 'rest/admin/cmmn-jobs/' + $scope.job.id + '?jobType=' + jobType.param
            }).success(function (data, status, headers, config) {
